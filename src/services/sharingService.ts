  ...
  async leaveBuilding(buildingLocalId: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (membership.role === "owner") {
        return {
          success: false,
          error: "Building owner cannot leave. Transfer ownership or delete the building instead.",
        }
      }

      // Remove membership
      const { error } = await supabase.from("building_members").delete().eq("id", membership.id)
